...
class DataPreparation:
    ...
    def add_missing_data_10_percent(self, df):
        ...
        ix = [(row, col) for row in range(df.shape[0]) for col in range((df.shape[1] - 1) if df.shape[1] > 1 else df.shape[1])]
        random.shuffle(ix)
        to_replace = int(round(.1 * len(ix)))
        for row, col in ix:
            if len(replaced[row]) < (df.shape[1] - 1 if df.shape[1] > 1 else df.shape[1]):
